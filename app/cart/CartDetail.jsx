...
const Cart = () => {
  ...
  const incrementQty = (cartItemId) => {
    incrementProductCartQty(cartItemId)
  }
  const decrementQty = (cartItemId) => {
    decrementProductCartQty(cartItemId)
  }
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-8">
            {loading
              ...
              {cart.map(item => (
                <li className="flex flex-col xs:flex-row items-center my-12 text-center xs:text-left xs:my-0 gap-4 " key={item.cartItemId}>
                  <Image
                  ...
                  <div >
                    <h3 className="text-sm font-bold line-clamp-2">{item.cartProductTitle}</h3>

                    <dl className="mt-0.5 space-y-px text-[10px] text-gray-600">
                      <div>
                        <dt className="inline text-gray-950 mr-2">{item.cartProductCategory}</dt>
                      </div>
                  ...
                  <div className="flex  flex-1 items-center justify-end gap-2">
                    <div className="flex items-center gap-1 ">
                      <button disabled={item.quantity === 1} onClick={() => decrementQty(item.cartItemId)} className="size-10 leading-10 text-gray-900  transition hover:opacity-75">
                        &minus;
                      </button>
                      ...
                      <button onClick={() => incrementQty(item.cartItemId)} className="size-10 leading-10 text-gray-900  transition hover:opacity-75">
                        +
                      </button>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <div className='font-bold text-sm '>
                        <p className='flex gap-1' ><span>$</span>{(item.cartProductPrice * item.quantity).toFixed(2)}</p>
                      </div>
                      <button onClick={() => deleteItem(item.cartItemId)} className="text-gray-600 transition hover:text-red-600">
